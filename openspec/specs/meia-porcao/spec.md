...
### Requirement: Item pode ter tamanho INTEIRA ou MEIA
O sistema SHALL permitir que cada item (seja `ItemPedido`, `ComandaItem` ou `ItemCompartilhado`) tenha um campo `tamanho` com valor `INTEIRA` (padrão) ou `MEIA`. O preço do item é calculado com base no tamanho e no preço do produto.

#### Scenario: Criar item com tamanho INTEIRA
- **WHEN** um garçom adiciona item (pedido, comanda ou compartilhado) sem especificar `tamanho`
- **THEN** o sistema assume `INTEIRA` e usa `produto.preco` para calcular o subtotal

#### Scenario: Criar item com tamanho MEIA
- **WHEN** um garçom adiciona item (pedido, comanda ou compartilhado) com `tamanho: MEIA`
...
#### Scenario: Listar item exibe tamanho
- **WHEN** um cliente/garçom consulta um pedido, comanda ou mesa
